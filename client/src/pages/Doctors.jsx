...
import { useDoctors } from '../data/useDoctors'
...
export default function Doctors() {
  ...
  // The directory is served by the API so that "accepting patients" reflects
  // each consultant's own diary. If the API is unreachable the bundled content
  // stands in, and the availability badge is suppressed rather than guessed at.
  const { doctors, live, loading } = useDoctors()
  ...
  const results = useMemo(() => {
    ...
    const list = doctors.filter((d) => {
      if (dept !== 'all' && d.departmentSlug !== dept) return false
      if (language !== 'all' && !d.languages.includes(language)) return false
      if (acceptingOnly && !d.isAccepting) return false
      if (!q) return true
      return [d.fullName, d.title, d.departmentName, ...d.focus].join(' ').toLowerCase().includes(q)
    })
  ...
  }, [doctors, query, dept, language, acceptingOnly, sort])
    ...
    <>
      ...
      <section className="bg-ivory py-14 lg:py-20">
        <Container size="wide">
            ...
            <div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-t border-stone-200 pt-6">
              ...
              <div className="flex items-center gap-5">
                ...
                <p className="flex items-center gap-2 text-[0.8125rem] text-pine-900/55">
                  ...
                  <span className="font-medium text-pine-900">{loading ? '…' : results.length}</span> consultant
                  {results.length === 1 ? '' : 's'}
                </p>
              </div>
            </div>
            ...
            <ul className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {results.map((doc, i) => (
                <Reveal as="li" key={doc.slug} delay={(i % 4) * 0.05}>
                  <DoctorCard doctor={doc} showAvailability={live} />
                </Reveal>
              ))}
            </ul>
          ) : (
          ...
          {!live && !loading && (
            <p className="mt-10 flex items-start gap-3 border border-stone-200 bg-parchment p-4 text-[0.8125rem] text-pine-900/60">
              <WifiOff className="mt-0.5 size-4 shrink-0 text-brass-500" aria-hidden="true" />
              Showing our published directory. Live availability could not be reached just now, so we have not
              shown who is currently taking bookings — please call us to check.
            </p>
          )}
