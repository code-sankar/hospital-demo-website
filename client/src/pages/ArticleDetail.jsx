...
import { staticDoctor } from '../data/doctorShape'
...
export default function ArticleDetail() {
  ...
  const author = staticDoctor(article.authorSlug)
            ...
            <div className="mt-10 flex items-center gap-4 border-t border-ivory/12 pt-8">
              {author && (
                <Link
                  ...
                  aria-label={`Read the profile of ${author.fullName}`}
                  className="size-14 shrink-0 overflow-hidden rounded-full ring-1 ring-brass-400/40"
                >
                  <Portrait seed={author.portraitSeed} showMonogram={false} name={author.fullName} />
                </Link>
              )}
              <div>
                {author ? (
