...
import { getDoctor, doctorsByDepartment } from '../data/doctors'
...
export default function DoctorDetail() {
  ...
  const dept = getDepartment(doctor.department)
  const colleagues = doctorsByDepartment(doctor.department).filter((d) => d.slug !== doctor.slug)
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-pine-900">
        <div className="absolute inset-0 opacity-35" aria-hidden="true">
          <ArtPanel variant={dept?.art ?? 'colonnade'} tone="deep" seed={doctor.seed} arch={false} />
        </div>
        ...
        <Container size="wide" className="relative">
          <div className="grid gap-12 py-14 lg:grid-cols-[0.65fr_1.35fr] lg:gap-16 lg:py-20">
            ...
            <div className="order-1 lg:order-2">
              ...
              <Link
                to={`/centres/${doctor.department}`}
                className="eyebrow mt-8 inline-block text-brass-300 transition-colors hover:text-ivory"
              >
                {doctor.departmentName}
              </Link>

              <h1 className="mt-5 text-[2.3rem] leading-[1.06] text-ivory sm:text-[3rem]">{doctor.name}</h1>
              ...
              <ul className="mt-8 flex flex-wrap items-center gap-x-8 gap-y-3 text-[0.875rem] text-pine-100/70">
                <li className="flex items-center gap-2">
                  ...
                  <span className="text-pine-100/45">({doctor.reviews} reviews)</span>
                </li>
                <li className="flex items-center gap-2">
              ...
              <aside className="border border-stone-200 bg-parchment p-8 lg:sticky lg:top-32">
                ...
                <ul className="mt-6 divide-y divide-stone-200 border-y border-stone-200">
                  {doctor.availability.map((slot) => (
                    <li key={slot} className="flex items-center justify-between gap-4 py-3.5 text-[0.9375rem] text-pine-900/78">
                      {slot}
                    </li>
                  ))}
                </ul>
      ...
      <CTABand
        ...
        title={`Request a consultation with ${doctor.name}`}
